...
bool shuffle_simple_test(int argc, char **argv)
{
	...
	shfl_scan_test<<<gridSize, blockSize, shmem_sz>>>(d_data, 32,
							  d_partial_sums);
	shfl_scan_test<<<p_gridSize, p_blockSize, shmem_sz>>>(d_partial_sums,
							      32);
...
bool shuffle_integral_image_test()
{
	...
	checkCudaErrors(cudaMalloc((void **)&d_data, sz));
	checkCudaErrors(cudaMalloc((void **)&d_integral_image,
	...
	checkCudaErrors(cudaMemcpy(h_image, d_integral_image, sz,
				   cudaMemcpyDeviceToHost));
	...
	cudaEventRecord(start);
	shfl_vertical_shfl<<<testGrid, blockSz>>>(
		(unsigned int *)d_integral_image, w, h);
	cudaEventRecord(stop);
	checkCudaErrors(cudaEventSynchronize(stop));
	checkCudaErrors(cudaEventElapsedTime(&et, start, stop));
